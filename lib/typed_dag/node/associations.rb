  ...
  module Associations
    ...
    included do
      ...
      _dag_options.types.each do |key, config|
        ...
        if from_one_limited
          has_one :"#{from_name}_relation", dag_relations_association_lambda(key, 1), {
            class_name: _dag_options.edge_class_name,
            foreign_key: _dag_options.to_column
          }
          ...
          has_many :"#{from_name}_relations", dag_relations_association_lambda(key, 1), {
            class_name: _dag_options.edge_class_name,
            foreign_key: _dag_options.to_column
          }
        ...
        has_many :"#{config[:to]}_relations", dag_relations_association_lambda(key, 1), {
          class_name: _dag_options.edge_class_name,
          foreign_key: _dag_options.from_column
        }
        ...
        has_many :"#{config[:all_to]}_relations", dag_relations_association_lambda(key), {
          class_name: _dag_options.edge_class_name,
          foreign_key: _dag_options.from_column
        }

        has_many config[:all_to], -> { distinct }, through: :"#{config[:all_to]}_relations", source: :to

        has_many :"#{config[:all_from]}_relations", dag_relations_association_lambda(key), {
          class_name: _dag_options.edge_class_name,
          foreign_key: _dag_options.to_column
        }

        has_many config[:all_from], -> { distinct }, through: :"#{config[:all_from]}_relations", source: :from
      end
    end
  end
end
